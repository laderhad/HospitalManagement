...
namespace HospitalManagement.Application.LabResults.Queries.GetMyDoctorLabResults;

[Authorize(Roles = Roles.Doctor)]
public record GetMyDoctorLabResultsQuery : IRequest<IReadOnlyCollection<LabResultSummaryDto>>;

public class GetMyDoctorLabResultsQueryHandler : IRequestHandler<GetMyDoctorLabResultsQuery, IReadOnlyCollection<LabResultSummaryDto>>
{
    private readonly IApplicationDbContext _context;
    private readonly IUser _user;

    public GetMyDoctorLabResultsQueryHandler(IApplicationDbContext context, IUser user)
    {
        _context = context;
        _user = user;
    }

    public async Task<IReadOnlyCollection<LabResultSummaryDto>> Handle(GetMyDoctorLabResultsQuery request, CancellationToken cancellationToken)
    {
        ...
        return await _context.LabResults
            .AsNoTracking()
            .Where(lr => lr.LabRequestItem!.LabRequest!.Examination!.Appointment!.DoctorId == doctorId)
