...
namespace HospitalManagement.Application.Examinations.Queries.GetMyDoctorExaminations;

[Authorize(Roles = Roles.Doctor)]
public record GetMyDoctorExaminationsQuery : IRequest<IReadOnlyCollection<ExaminationSummaryDto>>;

public class GetMyDoctorExaminationsQueryHandler : IRequestHandler<GetMyDoctorExaminationsQuery, IReadOnlyCollection<ExaminationSummaryDto>>
{
    private readonly IApplicationDbContext _context;
    private readonly IUser _user;

    public GetMyDoctorExaminationsQueryHandler(IApplicationDbContext context, IUser user)
    {
        _context = context;
        _user = user;
    }

    public async Task<IReadOnlyCollection<ExaminationSummaryDto>> Handle(GetMyDoctorExaminationsQuery request, CancellationToken cancellationToken)
    {
        ...
        return await _context.Examinations
            .AsNoTracking()
            .Where(e => e.Appointment!.DoctorId == doctorId)
